...
import { addTransaction } from '../../store/slice/dataSlice'
import { X } from 'lucide-react'

function AddModal({ onClose }) {
    const dispatch = useDispatch()
    const [form, setForm] = useState({
        description: '', amount: '', category: 'Food', type: 'expense', date: ''
    })
    ...
    const handleSubmit = () => {
        ...
        dispatch(addTransaction({ ...form, amount: Number(form.amount), id: Date.now() }))
        onClose()
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 relative">
                <div className="flex justify-between items-center mb-5">
                    <h2 className="text-[18px] font-bold text-[#0F1117]">Add Transaction</h2>
                    <X className="cursor-pointer text-[#64748B] hover:text-[#EF4444]" onClick={onClose} />
                </div>
                <div className="flex flex-col gap-4">
                    <div className="flex flex-col gap-1">
                        <label className={labelStyle}>Description</label>
                        <input name="description" value={form.description} onChange={handleChange} placeholder="e.g. Grocery Shopping" className={inputStyle} />
                    </div>
                    <div className="flex flex-col gap-1">
                        <label className={labelStyle}>Amount</label>
                        <input name="amount" type="number" value={form.amount} onChange={handleChange} placeholder="e.g. 1500" className={inputStyle} />
                    </div>
                    <div className="flex flex-col gap-1">
